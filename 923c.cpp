...
void solve(){
    ...
    if(map.size()>=k){
        ...
        for(auto &it : map){
            if(it.second==0) cnt0++;
            else if(it.second==1) cnt1++;
            else cnt2++;
            //cout<<it.first<<" "<<it.second<<endl;
        }
        if(cnt1>=k/2 && cnt2>=k/2) yes();
        else if(cnt1>=k/2){
            if(cnt2+cnt0>=k/2) yes();
            else no();
        }
        else if(cnt2>=k/2){
            if(cnt1+cnt0>=k/2) yes();
            else no();
        }
        else{
            if(cnt1+cnt2+cnt0>=k) yes();
            else no();
        }
    }
    else{
