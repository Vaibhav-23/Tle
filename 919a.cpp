...
void solve(){
   ...
   for(ll i=0;i<n;i++){
    if(a[i].first==1){
        curr.first=max(curr.first,a[i].second);
    }
    else if(a[i].first==2){
        curr.second=min(curr.second,a[i].second);
    }
    else{
        no.insert(a[i].second);
    }
   }
    ll cnt=0;
    for(auto &it:no){
        if(it>=curr.first && it<=curr.second) cnt++;
    }
    ll final=(curr.second-curr.first+1)-cnt;
    if(final<0) final=0;
    print(final);
}
